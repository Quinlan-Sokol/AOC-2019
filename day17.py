...
put = [ord(x) for x in "A,B,A,C,B,C,B,C,A,C\nL,10,R,12,R,12\nR,6,R,10,L,10\nR,10,L,10,L,12,R,6\nn\n"]
base = 0
i = 0
string = ""
heap = []
while dct[i] != 99:
    ...
    if op == 1:
        ...
        string += "#" if out == 35 else "." if out == 46 else "\n" if out == 10 else " "
        i += 2
    elif op == 5:
...
'''num = 0
grid = [list(x) for x in string[:-2].split("\n")]
for y in range(1, len(grid)-1):
    for x in range(1, len(grid[0])-1):
        if [grid[y][x], grid[y+1][x], grid[y-1][x], grid[y][x+1], grid[y][x-1]].count("#") == 5:
            num += x * y
